...
public class PartEngine extends APart{
			...
			if(vehicleOn != null){
				...
				for(APart part : childParts){
					if(part instanceof PartPropeller){
						...
						havePropeller = true;
						Point3d propellerThrustAxis = new Point3d(0D, 0D, 1D).rotateCoarse(propeller.localOrientation.copy().add(vehicleOn.orientation));
				...
				if(definition.engine.jetPowerFactor > 0){
					Point3d engineThrustAxis = new Point3d(0D, 0D, 1D).rotateCoarse(localOrientation.copy().add(vehicleOn.orientation));
	...
	public Point3d getForceOutput(){
		...
		if(definition.engine.jetPowerFactor > 0 && state.running){
			...
			engineForce.add(new Point3d(0D, 0D, thrust).rotateCoarse(localOrientation));
		}
