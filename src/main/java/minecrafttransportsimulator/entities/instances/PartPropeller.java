...
public class PartPropeller extends APart{	
	...
	public Point3d getForceOutput(){
		...
		if(connectedEngine != null && connectedEngine.state.running){
			...
			Point3d propellerThrustAxis = new Point3d(0D, 0D, 1D).rotateCoarse(localOrientation.copy().add(entityOn.orientation));
			...
			if(desiredLinearVelocity != 0){
				...
				propellerForce.add(new Point3d(0D, 0D, thrust).rotateCoarse(localOrientation));
			}
		}
